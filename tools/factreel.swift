...
func render(_ ctx: CGContext, _ t: Double) {
    ...
    // mark, static
    let mark = NSMutableAttributedString(attributedString: attr("Copius", font(serif, 38), rgb(INK)))
    mark.append(attr(" \u{2014} " + C.mark, font(serif, 38), rgb(INK2)))
    ctx.textPosition = CGPoint(x: 84, y: CGFloat(H) - 104 - 38); CTLineDraw(CTLineCreateWithAttributedString(mark), ctx)
    ...
    y += drawImage(ctx, drawing, top: y, width: 600, alpha: CGFloat(prog(t, 0, 0.3)), scale: CGFloat(0.72 + 0.28 * pop), dy: bob)
    y += 36

    // name
    let pn = outCubic(prog(t, 0.3, 0.35))
    y += draw(ctx, attr(C.name, font(sans, 34), rgb(INK3), spacing: 4), top: y, width: 912, alpha: CGFloat(pn), rise: CGFloat(-14 * (1 - pn)))
    y += 22

    // hook: line by line, staggered, rising; then the underline draws itself under the key word
    let hookAttr = attr(C.hook, font(serif, 92), rgb(INK), lineHeight: 101)
    let L = layout(hookAttr, top: y, width: 912)
    ...
    for (i, line) in L.lines.enumerated() {
        ...
        if keyRange.location != NSNotFound && keyRange.location >= sr.location && keyRange.location < sr.location + sr.length {
            ...
            if u > 0 {
                ...
                ctx.saveGState(); ctx.setStrokeColor(rgb(ACCENT)); ctx.setLineWidth(7); ctx.setLineCap(.round)
    ...
    draw(ctx, attr(C.line, font(sans, 40), rgb(INK2), lineHeight: 58), top: y, width: 860, alpha: CGFloat(pl), rise: CGFloat(-18 * (1 - pl)))
